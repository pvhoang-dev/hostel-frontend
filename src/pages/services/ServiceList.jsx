...
const ServiceList = () => {
    ...
    const columns = [
        ...
        { accessorKey: "default_price", header: "Default Price", cell: ({row}) => row.original.default_price?.toLocaleString(), enableSorting: true },
        ...
        { accessorKey: "created_at", header: "Created At", cell: ({row}) => new Date(row.original.created_at).toLocaleDateString(), enableSorting: true },
        {
            accessorKey: "actions",
            header: "Actions",
            cell: ({ row }) => (
    ...
    const updateSearchParams = (newParams) => {
        ...
        Object.keys(updatedParams).forEach(key => {
            if (!updatedParams[key] && key !== 'page') { // Keep page=1 even if it's the only param
